...
Como os índices i, j e k foram declarados fora dos escopos dos loops, o openmp tratou essas variáveis como compartilhadas
entre todas as thread, quando na verdade deveriam ser tratadas como variáveis privadas de cada thread. Dessa maneira, 
gerou-se condição de corrida com esses índices. Por isso, deve-se explicitar nas cláusulas o comportamento desejado pelo openmp,
por exemplo, "#pragma omp parallel for schedule(dynamic, 1), private(i, j, k)", ou simplesmente declarar esse índices de 
iteração dentro do escopo de cada loop.

*/

void mult_matrix(double* out, double* left, double *right, 
                 int rows_left, int cols_left, int cols_right) {
    //int i, j, k; //---> Causa da condição de corrida com índices de iteração.
