...
sem_t sem_cabeceiras[2];		// [0] = continente | [1] = ilha

unsigned long saiu_ponte;			// quantos veículos ja sairam da ponte
pthread_mutex_t mutex_saiu_ponte;	// cŕitico		

/* Inicializa a ponte. */
void ponte_inicializar() {

	sem_init(&sem_cabeceiras[1],0,0);
	sem_init(&sem_cabeceiras[0],0,veiculos_turno);

	pthread_mutex_init(&mutex_saiu_ponte,NULL);
...
void ponte_finalizar() {
	...
	sem_destroy(&sem_cabeceiras[0]);
    sem_destroy(&sem_cabeceiras[1]);
	pthread_mutex_destroy(&mutex_saiu_ponte);
